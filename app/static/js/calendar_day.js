...
function calendar_day_ajax(day){
    calendar_ajax_POST(day);
    calendar_ajax_GET(day);
}

function calendar_ajax_GET(day) {
    //window.alert('GET開始');
    $(function () {
        $.ajax({
            type: "GET",
            url: "/main/day_Ajax_GET",
            dataType: "json",
        })
            .done(function (data) {
                yoyaku = data;
...
function calendar_ajax_POST(day) {
    json = JSON.stringify(day);  // object型からJSON文字列(string型)に変換
    //window.alert(json)
    $.ajax({
        type: "POST",
        url: "/main/day_Ajax_POST",
        data: json,
